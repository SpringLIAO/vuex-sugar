...
* [API](#api)
    * [构造函数](#constructor)
    * [Vapi实例选项](#instance-options)
    * [Vapi实例方法](#instance-methods)
...
1. Import `v-api`(我叫做`Vapi`)。

2. 创建一个`Vapi`实例。

    创建实例时，可以传入实例选项。建议你应该始终传入一个`baseUrl`来表示向哪里发起请求。另外，可选的传入`axios`实例，也可以初始化默认的`state`。如果没有设置默认的`state`，`v-api`会自动根据`property`设置为`null`。
...
4. 生成store对象。
    ...
    手动调用`Vapi`实例的`getStore`方法，或者将`Vapi`实例传入`createStore`方法。
...
在`v-api`中需要明白三个重要的概念`Vapi`实例、`action`、`store`。

- `Vapi`实例：是通过`Vapi`构造函数与选项参数生成的对象，它主要包含了一些请求处理参数，默认的`state`，以及`action`的部分预定义参数。

- `action`：通过`Vapi`实例的`add`（或者其他快捷方法，如：get）方法添加的`action`。此处的`action`不同于Vuex中的`action`，这里仅仅只是包含`action`的一些预定义，并没有真正生成`action`函数。

    同样可以传入`action选项`，每调用一次`add`方法，都会产生一个新的`action`。执行`action`时，可以传入提交参数。

- `store`：这里的是`store`与Vuex store（module）的概念是一致的。通过调用`Vapi.getStore()`产生真正的`action`、`mutation`等。
...
> 除预留的提交参数之外，甚至可以提交其他任意的数据。这些数据将与请求结果一起被传递到`resolved/rejected`回调、`successHandler/errorHandler`等地方。但是这些数据并不会被`v-api`自动处理。
>
> 详情请参考[Api](#api)说明。
...
此时，`Vapi`将不会自动改变`state`，而是使用指定的`successHandler`。

### <a id="plain-action">生成普通action</a>

`Vapi`不仅能添加HTTP请求`cation`，也能发起一个普通的`action`。
...
`Vapi`使用`meta`参数为`headers`和`path`提供元数据。`meta`可以在提交`action`时指定，也可以在`action`定义时指定，还可以在`Vapi`实例上指定。
...
`meta`会自动合并`Vapi`实例、`action`定义及`action`提交上的`meta`字段。

`headers`与`path`相同。

### <a id="callback">使用回调</a>

`Vapi`允许使用`resovled`与`rejected`参数指定请求成功与失败时的回调。回调可以发起另一个`action`，也可以执行一个函数。
...
- **Array类型**： 回调执行集合，集合中的元素可以是以上任一合法的类型。`Vapi`将自动按顺序执行。

`resovled`与`rejected`参数可以分别在`Vapi`实例选项指定、`action`选项指定、`action`提交参数指定。`v-api`自动使用`contact`方法进行合并。
...
- **用法**: 
    ...
    `Vapi`方法内部会隐式的使用`new`创建一个对象。可以传入一个实例选项对象。

### <a id="instance-options">Vapi实例选项</a>
...
- **默认值**: 
    ...
    ```
- **详细**: `v-api`默认服务端REST API返回的结果形式如下：
    ```js
    {
        code: number,
        data: any,
...
### <a id="instance-methods">Vapi实例方法</a>
...
- **用法**:
    ...
    在`Vapi`实例上添加一个`action`，返回该实例。添加`action`并不是真正的Vuex action，这里添加了`action`的定义。可以通过选项对象设置`action`。
...
- **用法**: 
    ...
    添加`action`元数据，与`Vapi实例选项`用法一致，将与实例选项进行合并。
...
- **用法**:
    ...
    设置当前`action`的`resolved`，用于设置请求成功的回调。与`Vapi实例选项`用法一致，将与实例选项进行合并。
...
- **用法**
    ...
    `Vapi`实例的`getStore`调用了该方法，从`Vapi`实例创建一个store对象。
